...
#create a vector for time series  0- 8 yrs
time_yr <- seq(from = 0, to = 8, by = 1) 

#empty matrix for pop size output
proj_year <- matrix(ncol = length(fish_year0), nrow = length(time_yr))  

#fills year 0 into our matrix 
proj_year[1, ] <- fish_year0 

# Goes year by year (seq_along(time_yr)) and fills in the estimated population size base on previous year size  
for (i in 2:length(time_yr)) {   # start with yr1
...
proj_df <- data.frame(time_yr, proj_year) %>% 
  pivot_longer(cols = -time_yr, names_to = "lifestage", values_to = "stage_population") #covert wide to long
